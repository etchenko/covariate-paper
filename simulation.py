...
def run_simulation(df, Z, A, Y, model_type, selected_features, verbose, target):
    ...
    ACE = model.ipw(df2, A, Y, best_features)
    acc = model.accuracy(df3, A, best_features)
    var = model.conf_int(df2, A, Y, best_features, 100)

    # Compute ACE, accuracy, and varance using inputted adjustment set
    ACE2= model.ipw(df2, A, Y, selected_features)
    acc2 = model.accuracy(df3, A, selected_features)
    var2 = model.conf_int(df2, A, Y, selected_features, 100)

    if verbose:
        print(f'ACE: {round(ACE, 2)}, Accuracy: {round(acc, 2)}, Variance: {round(var, 2)} ')
        print(f'ACE: {round(ACE2, 2)}, Accuracy: {round(acc2, 2)}, Variance: {round(var2, 2)} ')

    return ACE, acc, var, ACE2, acc2, var2
...
def run_multiple_sims(type, graph, times = 1, nums = None):
    ...
    for _ in tqdm(range(times), desc=f"Running Simulations for {type}"):
        if not nums:
            nums = 1000 if graph == 1 else 2000 if graph == 3 else 10000
        df, za, zy, z = generateDag(nums, graph, 100)
        results.append(run_simulation(df, z, 'A', 'Y', type, zy, verbose = False, target = 'Y' if graph == 3 else 'A'))

    # Convert to array and compute column-wise averages
    results_array = np.array(results)
    averages = results_array.mean(axis=0)

    print(f"Model 1: Ace - {round(averages[0], 2)}, Accuracy - {round(averages[1], 2)}, Variance - {round(averages[2], 2)}")
    print(f"Model 2: Ace - {round(averages[3], 2)}, Accuracy - {round(averages[4], 2)}, Variance - {round(averages[5], 2)}")
